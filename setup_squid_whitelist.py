...
""".format(domain=domain, port=port, ssl_bump_mode=ssl_bump_mode,
    ...
    # Different access rules for SSL Bump vs non-SSL Bump
    if ssl_bump_mode != "off":
        # SSL Bump mode: Allow all CONNECT, filtering happens via ssl_bump based on SNI
        config += """# SSL Bump mode: Allow all HTTPS CONNECT requests
...
"""
        ...
        # Non-SSL Bump mode: Filter based on CONNECT destination
        config += """# Allow whitelisted domain only (HTTP and HTTPS)
http_access allow whitelist_domain
...
acl whitelist_sni ssl::server_name {domain}
